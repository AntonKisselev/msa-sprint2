...
echo "🏁 Регрессионный тест до миграции Hotelio-booking"
...
# отключено, из-за ошибки в самом монолите
...
# отключено ниже, потому как нет данных как внедриться в монолит для проверки пользователя и отеля
